function play(){
  ...
  document.getElementById('unfairfalls').innerHTML = null;
  var pointer;
  var player;
  var otherPlayers;
  var collisionGroup;
  ...
  function addPlayer(playerData){
      ...
      player.username = game.add.text(player.x, player.y-50, playerData.username, { font: "20px Arial", fill: "#fff", wordWrap: true, wordWrapWidth: player.width, align: "center"});
  ...
  function addOtherPlayer(playerData){
      ...
      otherPlayer.username = game.add.text(otherPlayer.x, otherPlayer.y-50, playerData.username, { font: "20px Arial", fill: "#fff", wordWrap: true, wordWrapWidth: otherPlayer.width, align: "center"});;
      otherPlayer.anchor.setTo(0.5);
      game.physics.p2.enable([ otherPlayer ], false);
      ...
      otherPlayersRef[playerData.id] = otherPlayers.children.length -1;
  }
  ...
  function controlOtherPlayer(otherPlayer, playerData){
      ...
      */
      ...
      moveUsernameTag(otherPlayer);
  }

  function moveUsernameTag(player){
    player.username.x = player.body.x - player.username.text.length * 4;
    player.username.y = player.body.y -60;
  }
  ...
  function handleSockets(){
      ...
      socket.on('currentPlayers', function (players) {
          ...
          Object.keys(players).forEach(function (playerId) {
              ...
              playerData.id === socket.id ? addPlayer(playerData) : addOtherPlayer(playerData);
          });
      });
